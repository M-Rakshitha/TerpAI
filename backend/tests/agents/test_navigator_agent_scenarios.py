...
@pytest.mark.asyncio
async def test_navigator_agent_defaults_to_stamp_when_query_is_empty(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert result["agent"] == "navigator"
    assert result["destination"] == "Stamp Student Union"
    assert "map.umd.edu" in result["map_url"]
...
async def test_navigator_agent_preserves_unknown_explicit_destination(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "building=Quantum+Cafe" in result["map_url"]
...
async def test_navigator_agent_builds_start_stop_map_url_when_origin_is_given(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "start=AVW" in result["map_url"]
    assert "stop=CSI" in result["map_url"]
